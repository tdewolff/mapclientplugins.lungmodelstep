    ...
    def get_unique_id(self, random_chars=0):
        ...
        if random_chars > 0:
            ...
            while random_id in existing_ids:
                random_id = ''.join(random.choice(string.ascii_letters + string.digits) for x in range(random_chars))
            return random_id
        else:
            while True:
...
class Core():
    ...
    def __init__(self):
        ...
        self.gauss_points = {}
        self.gauss_points[2] = [numpy.array([[0.21132486540518708], [0.78867513459481287]]), numpy.array([0.5, 0.5])]
        self.gauss_points[3] = [numpy.array([[0.1127016653792583], [0.5], [0.8872983346207417]]),
            numpy.array([5. / 18., 4. / 9., 5. / 18])]
        self.gauss_points[4] = [
            numpy.array([[0.33000947820757187, 0.6699905217924281, 0.06943184420297371, 0.9305681557970262]]).T,
        ...
        self.gauss_points[5] = [numpy.array([[0.5, 0.230765344947, 0.769234655053, 0.0469100770307, 0.953089922969]]).T,
                                numpy.array(
                                    [0.284444444444, 0.23931433525, 0.23931433525, 0.118463442528, 0.118463442528])]
        self.gauss_points[6] = [numpy.array([[0.8306046932331322, 0.1693953067668678, 0.3806904069584016,
                                              0.6193095930415985, 0.0337652428984240, 0.9662347571015760]]).T,
                                numpy.array(
                                    [0.1803807865240693, 0.1803807865240693, 0.2339569672863455, 0.2339569672863455,
                                     0.0856622461895852, 0.0856622461895852])]
    ...
    def generate_fixed_index(self):
        self.idx_unfixed = numpy.array([i for i, f in enumerate(self.fixed) if f == False])
    ...
    def get_gauss_points(self, ng):
        if isinstance(ng, int):
            ...
            if len(ng) == 2:
                ...
                gindex = numpy.array(
                    [gindex[2, :, :].flatten(), gindex[1, :, :].flatten(), gindex[0, :, :].flatten()]).T
                Xi = numpy.array([Xi1[gindex[:, 0]], Xi2[gindex[:, 1]], Xi3[gindex[:, 2]]])[:, :, 0].T
                W = numpy.array([W1[gindex[:, 0]], W2[gindex[:, 1]], W3[gindex[:, 2, ]]]).T.prod(1)
                return Xi, W
    ...
    def add_pca_node(self, pca_node):
        self.PCAMap.append(
            [pca_node.cids, pca_node.node.shape, pca_node.node.cids, pca_node.weights.cids, pca_node.variance.cids])
        return len(self.PCAMap) - 1

    def update_pca_nodes(self):
        for pcamap in self.PCAMap:
            # self.P[pcamap[0]] = self.P[pcamap[0]].reshape(3, 4)
            self.P[pcamap[0]] = numpy.dot(self.P[pcamap[2]].reshape(pcamap[1]),
                self.P[pcamap[3]] * self.P[pcamap[4]]).reshape(12, )
